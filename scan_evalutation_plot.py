...
def get_params(args, analysis=False):
    if not analysis:
        pairs = [
            ('Standard', 'logs/baseline_Standard_scan.', ('orange', 'v')),
            ('Compositional', 'logs/baseline_Compositoinal_scan.', ('r', '^')),
            ('EWC', 'logs/baseline_EWC_scan.', ('b', 'D')),
            ('MAS', 'logs/baseline_MAS_scan.', ('c', 's')),
            ("$\mathbf{Proposed}$", 'logs/main_proposed_scan.', ('g', 'o')),
        ]
        file_list = [x[1] for x in pairs]
        legends = [x[0] for x in pairs]
        colors = [x[2] for x in pairs]
        output_list = [
            'outputs_scan/compositional_generalization_results',
            'outputs_scan/catastrophic_forgetting_results',
            'outputs_scan/long_term_catastrophic_forgetting_results',
            'outputs_scan/initial_results']
        lw = 2
        loc = 'center right'
    else:
        pairs = [
        ...
        colors = [x[2] for x in pairs]
        output_list = [
            'outputs/compositional_generalization_analysis',
            'outputs/catastrophic_forgetting_analysis',
            'outputs/long_term_catastrophic_forgetting_analysis',
